...
export default function Layout({ children }) {
  const { latesSetNotifications, setNewCount,setUserMessages,setnewInvoice,newInvoiceCout,setNewInvoiceCount } = useNotifications(); 
  ...
  useEffect(() => {
    ...
    const startSocketEvents = () => {
      ...
      socket.on("admin-message",(message)=>{setUserMessages(message)})
      socket.on("notification", ({ data }) => handleNotification({ notification: data }));
